...
function widgetCountDown($wNumber, $name, $customOption, $positionX , $positionY ,$width , $height, $reflection){
	//$flashtag .= '';
...
function wp_CountDown_createflashcode( $widget=false, $wNumber, $height, $width, $addSettings ,$scalesize ,$containerx, $reflection ){
	$mainoptions = get_option('wpCountDown_options');
	// write flash 
	if($addSettings == '0'){
	$flashtag = widgetCountDown($wNumber, 'CountDown', $addSettings, $mainoptions['ContainerX'], $mainoptions['ContainerY'] , $width, $height, $reflection );	
	}
	if($addSettings == '1'){
	$flashtag = widgetCountDown($wNumber, 'CountDown', $addSettings, $containerx , $containery  , $width, $height, $reflection );	
	}
...
//      width and height without px only number
	...
	if($custom_widget == '0'){
		...
		$flashtagShort = widgetCountDown($id, 'CountDownShort', $custom_widget, $containerx, $containery , $width, $height ,$reflection );	
	}
	if($custom_widget == '1'){
		XMLCountDown($id, $scalesize, $containerx ,$containery  ,$numbermargin ,$reflection, $year,$month,$day,$hours,$minute,$imagebackground,$textunder);
		$flashtagShort = widgetCountDown($id, 'CountDownShort', $custom_widget, $containerx , $containery  , $width, $height , $reflection );	
...
function wp_CountDown_php($id,$year,$month,$day,$hours,$minute){
	...
	$flashtagShort = widgetCountDown($id, 'CountDownPhp', 0, $mainoptions['ContainerX'], $mainoptions['ContainerY'] , $mainoptions['width'], $mainoptions['height'] ,$mainoptions['reflection'] );	
...
function widget_CountDown($args, $vars = array()) {
	...
	if( !stristr( $_SERVER['PHP_SELF'], 'widgets.php' ) ){
		echo wp_CountDown_createflashcode(true,$vars['id'],$vars['height'],$vars['width'],$vars['addSettings'] ,$vars['scalesize'], $vars['containerx'] ,$vars['containery'], $vars['reflection'] );
