...
function wp_CountDown_install () {
	...
	$newoptionsCountDown['color'] = 'dark';	
	...
	$newoptionsCountDown['secondstext'] = 'Second';
...
function widgetCountDown($wNumber, $name, $customOption, $positionX , $positionY ,$width , $height, $reflection, $color ){
	//$flashtag .= '';
	...
	if($reflection == 'true'){	
		...
		if($color == 'dark')
			$flashFile = "wp-flash-countdown/counter_cs3_v2_reflect.swf";
		else
			$flashFile = "wp-flash-countdown/counter_cs3_v2_reflectLight.swf";	
	}
	else{
		$widthIn = (int)$width;
		$heightIn = $widthIn / 2.45;	
		if($color == 'dark')
			$flashFile = "wp-flash-countdown/counter_cs3_v2_NoReflect.swf";
		else
			$flashFile = "wp-flash-countdown/counter_cs3_v2_NoReflectLight.swf";		
	}
	...
	$flashtag = '<div style = "position:relative; width:'.$widthIn.'px; height:'.$heightIn.'px; margin-left:'.$positionXN.'px; margin-top:'.$positionYN.'px; text-align:center;"><script type="text/javascript" src="'.plugins_url("wp-flash-countdown/swfobject/swfobject.js").'" charset="utf-8"></script><script type="text/javascript" src="'.plugins_url("wp-flash-countdown/swfobject/swfaddress.js").'" charset="utf-8"></script><script type="text/javascript">
								...
								swfobject.embedSWF("'.plugins_url($flashFile).'", "'.$name.'-'.$wNumber.'", "'.$widthIn.'px", "'.$heightIn.'px", "9.0.0", "'.plugins_url("wp-flash-countdown/swfobject/expressInstall.swf").'", flashvars, params, attributes);
							</script>
...
function wp_CountDown_createflashcode( $widget, $wNumber, $height, $width, $addSettings ,$scalesize ,$containerx, $containery,  $reflection, $colorCount  ){
	$mainoptions = get_option('wpCountDown_options');
	// write flash 
	if($addSettings == '0'){
	$flashtag = widgetCountDown($wNumber, 'CountDown', $addSettings, $mainoptions['ContainerX'], $mainoptions['ContainerY'] , $width, $height, $mainoptions['reflection'], $mainoptions['color'] );	
	}
	if($addSettings == '1'){
	$flashtag = widgetCountDown($wNumber, 'CountDown', $addSettings, $containerx , $containery  , $width, $height, $reflection , $colorCount );	
	}
	return $flashtag;
}
...
//      width and height without px only number
	...
	if($custom_widget == '0'){
		...
		$flashtagShort = widgetCountDown($id, 'CountDownShort', $custom_widget, $containerx, $containery , $width, $height ,$reflection ,$color );	
	}
	if($custom_widget == '1'){
		XMLCountDown($id, $scalesize, $containerx ,$containery  ,$numbermargin ,$reflection, $year,$month,$day,$hours,$minute,$imagebackground,$textunder);
		$flashtagShort = widgetCountDown($id, 'CountDownShort', $custom_widget, $containerx , $containery  , $width, $height , $reflection ,$color );	
...
//php
function wp_CountDown_php($id,$year,$month,$day,$hours,$minute){
	$mainoptions = get_option('wpCountDown_options');
	XMLCountDown($id, 1, $mainoptions['ContainerX'] , $mainoptions['ContainerY']  , 0 , $mainoptions['reflection'], $year,$month,$day,$hours,$minute,$mainoptions['imagebackground'],$mainoptions['textUnder']);
	$flashtagShort = widgetCountDown($id, 'CountDownPhp', 0, $mainoptions['ContainerX'], $mainoptions['ContainerY'] , $mainoptions['width'], $mainoptions['height'] ,$mainoptions['reflection'] ,$mainoptions['color'] );	
...
function wp_CountDown_options() {	
	...
	if (!empty($_POST['wpCountDown_submit']))  {
		...
		$newoptionsCountDown['color'] = strip_tags(stripslashes($_POST["color"]));
	// if changes save!
...
?> 
    ...
    <b><font size="1">[CountDown id=1 width=250 custom_widget=1 containerx=0 containerxy=0 imagebackground=true reflection=true year=2011 month=7 day=25 hours=5 minute=10 textunder=true color=dark]</font></b><br><br>
	...
	If you want to use php code:<br>
	<b><font size="1">< ?php echo wp_CountDown_php(id,year,month,day,hours,minute); ?></font></b><br>
	Remove space betwean "<" and "?"!
	<br><br>
	...
	//color
	echo '<tr valign="top"><th scope="row">CountDown Color</th>';	
	echo '<td><input type="radio" name="color" value="dark"';
	if( $optionsCountDown['color'] == 'dark' ){ echo ' checked="checked" '; }
	echo '></input> Dark (if you select dark CountDown will be dark color)<br /><input type="radio" name="color" value="light"';
	if( $optionsCountDown['color'] == 'light' ){ echo ' checked="checked" '; }
	echo '></input> Light (if you select light CountDown will be light color)<br /></td></tr>';
	//reflection
...
function widget_CountDown($args, $vars = array()) {
	...
	if( !stristr( $_SERVER['PHP_SELF'], 'widgets.php' ) ){
		...
		echo  wp_CountDown_createflashcode(true,$vars['id'],$vars['height'],$vars['width'],$vars['addSettings'] ,$vars['scalesize'], $vars['containerx'] ,$vars['containery'], $vars['reflection'], $vars['colorCount'] );
...
function widget_CountDown_control($args) {
	...
	echo '<td><input type="text" name="'. $prefix.'['. $number .'][containery]" value="'.$containery.'" size="5"></input><br />Position of countdown on Y axis.</td></tr>';	
		//color
	echo '<tr valign="top"><th scope="row">CountDown Color</th>';	
	echo '<td><input type="radio" name="'. $prefix.'['. $number .'][colorCount]" value="dark"';
	if( $colorCount == 'dark' ){ echo ' checked="checked" '; }
	echo '></input>  Dark (if you select true reflection will be enabled)<br /><input type="radio" name="'. $prefix.'['. $number .'][colorCount]" value="light"';
	if( $colorCount == 'light' ){ echo ' checked="checked" '; }
	echo '></input>  Light (if you select false reflection will be disabled)<br /></td></tr>';
	//reflection
	echo '<tr valign="top"><th scope="row">Reflection</th>';	
